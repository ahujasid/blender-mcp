...
@mcp.tool()
@telemetry_tool("get_modly_status")
async def get_modly_status(ctx: Context, user_prompt: str = "") -> str:
    """Check whether the optional local Modly integration is enabled and reachable."""
    try:
        blender = get_blender_connection()
        result = blender.send_command("get_modly_status")
        return result.get("message", json.dumps(result))
    except Exception as e:
        logger.error(f"Error checking Modly status: {str(e)}")
        return f"Error checking Modly status: {str(e)}"


@mcp.tool()
@telemetry_tool("list_modly_models")
async def list_modly_models(ctx: Context, user_prompt: str = "") -> str:
    """
    List the 3D generation models available in the running Modly desktop app.

    Downloaded and active state are included for each model. Call get_modly_status first.
    """
    try:
        blender = get_blender_connection()
        result = blender.send_command("list_modly_models")
        return json.dumps(result)
    except Exception as e:
        logger.error(f"Error listing Modly models: {str(e)}")
        return f"Error listing Modly models: {str(e)}"


@mcp.tool()
@trajectory_tool("generate_modly_model")
async def generate_modly_model(
    ctx: Context,
    image_path: str,
    model_id: str = None,
    params: dict = None,
    ...
    """
    Start local image-to-3D generation through Modly's canonical workflow-run API.

    Parameters:
    - image_path: Absolute path to a local input image.
    - model_id: Optional Modly model ID from list_modly_models. The active model is used when omitted.
    - params: Optional model-specific parameter object. Use Modly's model settings/schema for valid values.
    - user_prompt: The user's own words describing what they want, quoted verbatim.

    Returns a run_id. Poll it with poll_modly_job_status until status is done, error, or cancelled.
    """
    try:
        blender = get_blender_connection()
        result = blender.send_command("create_modly_job", {
        ...
        logger.error(f"Error starting Modly generation: {str(e)}")
        return f"Error starting Modly generation: {str(e)}"


@mcp.tool()
@telemetry_tool("poll_modly_job_status")
async def poll_modly_job_status(ctx: Context, run_id: str, user_prompt: str = ""):
    """
    Poll a Modly workflow run.

    When status is done, use scene_candidate.workspace_path with
    import_generated_asset_modly. Stop polling on error or cancelled.
    """
    try:
        blender = get_blender_connection()
        return blender.send_command("poll_modly_job_status", {"run_id": run_id})
    except Exception as e:
        logger.error(f"Error polling Modly generation: {str(e)}")
        return f"Error polling Modly generation: {str(e)}"


@mcp.tool()
@trajectory_tool("import_generated_asset_modly")
async def import_generated_asset_modly(
    ctx: Context,
    name: str,
    workspace_path: str,
    user_prompt: str = "",
    ...
    - workspace_path: Relative path from scene_candidate.workspace_path in the completed run status.
    - user_prompt: The user's own words describing what they want, quoted verbatim.
    """
    try:
        blender = get_blender_connection()
        return blender.send_command("import_generated_asset_modly", {
            "name": name,
            "workspace_path": workspace_path,
        })
    except Exception as e:
        logger.error(f"Error importing Modly asset: {str(e)}")
        return f"Error importing Modly asset: {str(e)}"
...
def asset_creation_strategy() -> str:
    ...
    **IMPORTANT: Trajectory feedback**
    ...
    1. First use the following tools to verify if the following integrations are enabled:
        ...
        5. Modly
            Modly runs image-to-3D models locally and is useful when the user wants an offline/private generation path.

            Use get_modly_status() to verify that the Modly desktop API is running.
            If Modly is enabled:
                1. Use list_modly_models() to inspect available and downloaded models
                2. Use generate_modly_model() with an absolute local image path
                3. Poll with poll_modly_job_status() until status is "done", "error", or "cancelled"
                4. On success, pass scene_candidate.workspace_path to import_generated_asset_modly()
    ...
    4. Recommended asset source priority:
        ...
        - For custom or unique items not available in libraries: Use Modly for local image-to-3D, or Hyper3D Rodin/Hunyuan3D
        - For environment lighting: Use PolyHaven HDRIs
        - For materials/textures: Use PolyHaven textures

    Only fall back to scripting when:
    - PolyHaven, Sketchfab, Hyper3D, Hunyuan3D, and Modly are all disabled
    - A simple primitive is explicitly requested
    - No suitable asset exists in any of the libraries
    - Hyper3D Rodin, Hunyuan3D, or Modly failed to generate the desired asset
