...
@mcp.tool()
@telemetry_tool("get_comfyui_status")
async def get_comfyui_status(ctx: Context, user_prompt: str = "") -> str:
    """Check whether ComfyUI Desktop workflow integration is enabled and reachable."""
    try:
        blender = get_blender_connection()
        result = blender.send_command("get_comfyui_status")
        return result.get("message", json.dumps(result))
    except Exception as e:
        logger.error(f"Error checking ComfyUI Desktop status: {str(e)}")
        return f"Error checking ComfyUI Desktop status: {str(e)}"


@mcp.tool()
@trajectory_tool("run_comfyui_workflow")
async def run_comfyui_workflow(
    ...
    """
    Submit a local ComfyUI Desktop workflow exported in API format.

    Parameters:
    - workflow_path: Absolute path to JSON exported with ComfyUI's "Save (API Format)" command.
    - input_overrides: Optional object keyed by node id, then existing input name, for example
      {"6": {"text": "a wooden chair"}, "3": {"seed": 42}}.
    - input_image_path: Optional absolute path to an image that should be uploaded to ComfyUI.
    - input_image_node_id: Required with input_image_path; the LoadImage-style node to update.
    - input_image_field: Existing input name to update on that node. Defaults to "image".
    - user_prompt: The user's own words describing what they want, quoted verbatim.

    Returns a prompt_id. Poll it with poll_comfyui_workflow until done, error, or cancelled.
    """
    try:
        blender = get_blender_connection()
        result = blender.send_command("run_comfyui_workflow", {
            "workflow_path": workflow_path,
            "input_overrides": input_overrides,
            "input_image_path": input_image_path,
            "input_image_node_id": input_image_node_id,
            "input_image_field": input_image_field,
        })
        return json.dumps(result)
    except Exception as e:
        logger.error(f"Error running ComfyUI workflow: {str(e)}")
        return f"Error running ComfyUI workflow: {str(e)}"


@mcp.tool()
@telemetry_tool("poll_comfyui_workflow")
async def poll_comfyui_workflow(ctx: Context, prompt_id: str, user_prompt: str = ""):
    """
    Poll a ComfyUI Desktop workflow by prompt_id.

    A completed response contains importable 3D output references discovered in workflow history.
    Prefer a .glb output. Stop polling when status is done, error, or cancelled.
    """
    try:
        blender = get_blender_connection()
        return blender.send_command("poll_comfyui_workflow", {"prompt_id": prompt_id})
    except Exception as e:
        logger.error(f"Error polling ComfyUI workflow: {str(e)}")
        return f"Error polling ComfyUI workflow: {str(e)}"


@mcp.tool()
@trajectory_tool("import_comfyui_output")
async def import_comfyui_output(
    ctx: Context,
    name: str,
    filename: str,
    subfolder: str = "",
    ...
    """
    Download and import one 3D result returned by poll_comfyui_workflow.

    Parameters:
    - name: Name for the primary imported mesh object.
    - filename: Exact filename from the selected output reference.
    - subfolder: Exact subfolder from the output reference.
    - folder_type: Exact type from the output reference: input, output, or temp.
    - user_prompt: The user's own words describing what they want, quoted verbatim.

    GLB is preferred because it preserves embedded materials. GLTF, OBJ, FBX, STL, and PLY are also accepted.
    """
    try:
        blender = get_blender_connection()
        return blender.send_command("import_comfyui_output", {
            "name": name,
            "filename": filename,
            "subfolder": subfolder,
        ...
        logger.error(f"Error importing ComfyUI output: {str(e)}")
        return f"Error importing ComfyUI output: {str(e)}"
...
def asset_creation_strategy() -> str:
    ...
    **IMPORTANT: Trajectory feedback**
    ...
    1. First use the following tools to verify if the following integrations are enabled:
        ...
        4. ComfyUI Desktop
            ComfyUI Desktop can run a user-provided API-format workflow locally, including custom 3D workflows.

            Use get_comfyui_status() before starting a workflow.
            If ComfyUI Desktop is enabled:
                1. Use run_comfyui_workflow() with an absolute API-format workflow JSON path
                2. Apply only deliberate node input overrides; upload an input image when the workflow needs one
                3. Poll with poll_comfyui_workflow() until status is "done", "error", or "cancelled"
                4. Select a returned 3D output reference, preferably GLB, and pass its exact fields to import_comfyui_output()
                5. Check the imported mesh's world_bounding_box and adjust its location, scale, and rotation
        5. Hunyuan3D
    ...
    4. Recommended asset source priority:
        ...
        - For custom or unique items not available in libraries: Use a supplied ComfyUI workflow, Hyper3D Rodin, or Hunyuan3D
        - For environment lighting: Use PolyHaven HDRIs
        - For materials/textures: Use PolyHaven textures

    Only fall back to scripting when:
    - PolyHaven, Sketchfab, Hyper3D, Hunyuan3D, and ComfyUI Desktop are all disabled
    - A simple primitive is explicitly requested
    - No suitable asset exists in any of the libraries
    - ComfyUI Desktop, Hyper3D Rodin, or Hunyuan3D failed to generate the desired asset
