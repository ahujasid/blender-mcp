...
bl_info = {
    ...
    "description": "Connect Blender to MCP clients and visualise 3D stress simulations",
    "category": "Interface",
}
...
class BlenderMCPServer:
    ...
    def _execute_command_internal(self, command):
        ...
        handlers = {
            ...
            "visualize_stress_result": self.visualize_stress_result,
    ...
    def visualize_stress_result(self, result, deformation_scale=0.0, clear_previous=True,
                                show_loads=True, show_constraints=True):
        """Create a deformed, colour-mapped stress scene from solver output."""
        nodes = result.get("nodes", [])
        elements = result.get("elements", [])
        displacement = result.get("displacement_m", [])
        utilization = result.get("element_utilization", [])
        faces = result.get("faces", [])
        loads = result.get("loads_n", [])
        fixed_nodes = result.get("fixed_nodes", [])
        metrics = result.get("metrics", {})
        model = result.get("model", {})
        if not nodes or not elements or len(displacement) != len(nodes):
            raise ValueError("Stress result is missing nodes, elements or displacement")

        if clear_previous:
            for old_collection in list(bpy.data.collections):
                if old_collection.name.startswith("StressSim::"):
                    for obj in list(old_collection.objects):
                        bpy.data.objects.remove(obj, do_unlink=True)
                    bpy.data.collections.remove(old_collection)

        collection = bpy.data.collections.new(f"StressSim::{model.get('name', 'result')}")
        bpy.context.scene.collection.children.link(collection)
        minimum = [min(point[axis] for point in nodes) for axis in range(3)]
        maximum = [max(point[axis] for point in nodes) for axis in range(3)]
        diagonal = max(sum((maximum[i] - minimum[i]) ** 2 for i in range(3)) ** 0.5, 1e-6)
        max_displacement = max((sum(v * v for v in row) ** 0.5 for row in displacement), default=0.0)
        actual_scale = float(deformation_scale)
        if actual_scale <= 0:
            actual_scale = 0.12 * diagonal / max(max_displacement, diagonal * 1e-12)
        actual_scale = min(actual_scale, 1e6)
        deformed = [[nodes[i][a] + displacement[i][a] * actual_scale for a in range(3)]
                    for i in range(len(nodes))]

        ramp = ((0.03, 0.12, 0.55, 1.0), (0.00, 0.70, 1.00, 1.0),
                (0.10, 0.90, 0.45, 1.0), (1.00, 0.85, 0.05, 1.0),
                (1.00, 0.25, 0.02, 1.0))

        def color_at(value):
            value = max(0.0, min(1.0, float(value)))
            scaled = value * (len(ramp) - 1)
            lower = min(int(scaled), len(ramp) - 2)
            fraction = scaled - lower
            return tuple(ramp[lower][i] * (1 - fraction) + ramp[lower + 1][i] * fraction
                         for i in range(4))

        def get_material(name, color, metallic=0.0):
        ...
        max_utilization = max((abs(float(value)) for value in utilization), default=1.0) or 1.0
        bin_count = 12
        bins = [[] for _ in range(bin_count)]
        for index, pair in enumerate(elements):
            normalized = abs(float(utilization[index])) / max_utilization if index < len(utilization) else 0.0
            bins[min(int(normalized * (bin_count - 1)), bin_count - 1)].append(pair)

        member_radius = max(diagonal * 0.0035, 0.00008)
        for bin_index, members in enumerate(bins):
            if not members:
                continue
            curve = bpy.data.curves.new(f"StressMembers_{bin_index:02d}", type="CURVE")
            curve.dimensions = "3D"
            curve.bevel_depth = member_radius
            curve.bevel_resolution = 3
            obj = bpy.data.objects.new(curve.name, curve)
            collection.objects.link(obj)
            curve.materials.append(get_material(
                f"StressRamp_{bin_index:02d}", color_at(bin_index / (bin_count - 1)), 0.12))
            for node_i, node_j in members:
                spline = curve.splines.new("POLY")
                spline.points.add(1)
                spline.points[0].co = (*deformed[node_i], 1.0)
                spline.points[1].co = (*deformed[node_j], 1.0)

        ghost = bpy.data.curves.new("UndeformedReference", type="CURVE")
        ghost.dimensions = "3D"
        ghost.bevel_depth = member_radius * 0.25
        ghost_obj = bpy.data.objects.new(ghost.name, ghost)
        collection.objects.link(ghost_obj)
        ghost.materials.append(get_material("StressUndeformed", (0.12, 0.16, 0.22, 1.0)))
        for node_i, node_j in elements:
            spline = ghost.splines.new("POLY")
            spline.points.add(1)
            spline.points[0].co = (*nodes[node_i], 1.0)
            spline.points[1].co = (*nodes[node_j], 1.0)

        if faces:
            mesh = bpy.data.meshes.new("StressSurface")
            mesh.from_pydata(deformed, [], faces)
            mesh.update()
            surface = bpy.data.objects.new("StressSurface", mesh)
            collection.objects.link(surface)
            surface_material = get_material("StressSurface", (0.12, 0.30, 0.55, 1.0), 0.15)
            mesh.materials.append(surface_material)

        if show_constraints:
            for node_index in fixed_nodes:
                marker = bpy.data.objects.new(f"Fixed_{node_index}", None)
                marker.empty_display_type = "CUBE"
                marker.empty_display_size = diagonal * 0.018
                marker.color = (0.05, 0.85, 0.95, 1.0)
                marker.location = nodes[node_index]
                collection.objects.link(marker)

        if show_loads:
            max_load = max((sum(c * c for c in vector) ** 0.5 for vector in loads), default=0.0)
            if max_load > 0:
                load_curve = bpy.data.curves.new("AppliedLoads", type="CURVE")
                load_curve.dimensions = "3D"
                load_curve.bevel_depth = member_radius * 0.65
                load_obj = bpy.data.objects.new(load_curve.name, load_curve)
                collection.objects.link(load_obj)
                load_curve.materials.append(get_material("StressLoad", (1.0, 0.08, 0.45, 1.0), 0.1))
                for node_index, vector in enumerate(loads):
                    magnitude = sum(c * c for c in vector) ** 0.5
                    if magnitude <= max_load * 1e-8:
                        continue
                    length = diagonal * 0.14 * (0.35 + 0.65 * magnitude / max_load)
                    start = deformed[node_index]
                    end = [start[i] + vector[i] / magnitude * length for i in range(3)]
                    spline = load_curve.splines.new("POLY")
                    spline.points.add(1)
                    spline.points[0].co = (*start, 1.0)
                    spline.points[1].co = (*end, 1.0)

        text_curve = bpy.data.curves.new("StressSummary", type="FONT")
        text_curve.body = (
            f"{model.get('title', 'Stress simulation')}\n"
            f"Max stress  {metrics.get('max_abs_stress_pa', 0.0) / 1e6:.2f} MPa\n"
            f"Max displacement  {metrics.get('max_displacement_m', 0.0) * 1e3:.3f} mm\n"
            f"Min safety factor  {metrics.get('minimum_safety_factor', 0.0):.2f}\n"
            f"Deformation x{actual_scale:.2g} · screening model"
        )
        text_curve.size = diagonal * 0.035
        text_obj = bpy.data.objects.new("StressSummary", text_curve)
        text_obj.location = (minimum[0], minimum[1] - diagonal * 0.10,
                             maximum[2] + diagonal * 0.10)
        collection.objects.link(text_obj)
        text_curve.materials.append(get_material("StressText", (0.93, 0.96, 1.0, 1.0)))
        collection["solver"] = result.get("solver", "unknown")
        collection["deformation_scale"] = actual_scale
        collection["screening_only"] = True
        return {
            "collection": collection.name,
            "objects": len(collection.objects),
            "deformation_scale": actual_scale,
            "stress_range_pa": [0.0, float(metrics.get("max_abs_stress_pa", 0.0))],
        }
