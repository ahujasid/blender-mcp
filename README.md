...
**Connect Blender to any LLM — now with a transparent 3D stress lab**

Prompt-assisted 3D modeling, scene creation, and manipulation — driven by AI.

This fork is synchronised with upstream `ahujasid/blender-mcp` through commit
[`c69b901`](https://github.com/ahujasid/blender-mcp/commit/c69b90153616f2d767fe2e825d3310efbf6fcab5)
(2026-08-16) and adds deterministic linear 3D truss/FEM analysis, parameter
sweeps, engineering diagnostics and native Blender stress visualisation.
...
```bash
claude mcp add blender -- uvx --from git+https://github.com/waterfirst/blender-mcp.git blender-mcp
```
</details>
...
```bash
uvx --from git+https://github.com/waterfirst/blender-mcp.git blender-mcp install-addon
```
...
- [Features](#features)
- [3D Stress Simulator](#3d-stress-simulator)
...
| **3D stress simulation** | Linear 3D truss/FEM, thermal strain, parameter sweeps, reactions, safety factor and Blender heatmaps |

## 3D Stress Simulator

The stress lab is a reproducible engineering-screening layer rather than an
animation trick. It assembles a global stiffness matrix, solves constrained
nodal displacement, recovers axial strain/stress and checks numerical
conditioning and force equilibrium. Blender then builds the deformed model,
an undeformed reference, stress colours, constraints, load vectors and a KPI
panel without using the arbitrary-code MCP tool.

Available MCP tools:

- `list_stress_simulation_examples` — list 14 industry examples and 9 materials.
- `run_stress_simulation` — solve one case and optionally create the Blender scene.
- `sweep_stress_parameter` — compare up to 20 load or section-area values headlessly.

Try these prompts:

```text
List the stress simulation examples, then run foldable_oled_hinge and visualise it.
Run truss_bridge at load_scale 0.5, 1.0, 1.5 and compare displacement and safety factor.
Compare aluminium, steel and CFRP for robot_arm_reach using mass and tip displacement.
Show chip_package_warpage after a 125 °C cooldown and explain the CTE-mismatch assumptions.
```

The examples span beams, bridge, crane, robot arm, tower, pressure pipe,
bicycle, battery drop, smartphone/PCB bending, solar-panel wind, foldable OLED
and semiconductor package warpage. See the formulas, example matrix, controls,
limitations and validation plan in [3D Stress Simulator Guide](docs/STRESS_SIMULATOR.md).

> **Engineering boundary:** this is a linear axial-member model for concept
> screening, teaching and visual communication. It is not a validated
> solid/shell/contact solver and must not be used alone for certification or a
> safety-critical release decision.
...
- **Blender** 3.0 or newer; **Blender 5.2 LTS** is recommended and tested by design
...
> Optional: `uvx --from git+https://github.com/waterfirst/blender-mcp.git blender-mcp addon-paths` lists detected Blender addons folders. Override the destination with `BLENDERMCP_ADDONS_DIR=/path/to/scripts/addons`.
...
```bash
uvx --from git+https://github.com/waterfirst/blender-mcp.git blender-mcp install-addon
uvx --from git+https://github.com/waterfirst/blender-mcp.git blender-mcp addon-paths
```
...
```bash
DISABLE_TELEMETRY=true uvx --from git+https://github.com/waterfirst/blender-mcp.git blender-mcp
```
